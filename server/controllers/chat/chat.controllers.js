...
const Chat=require('../../models/chat/chat.models.js')
const ChatMessage=require('../../models/chat/message.models.js')
const ApiError=require('../../utils/ApiError.js')
const ApiResponse=require('../../utils/ApiResponse.js')
const asyncHandler=require('../../utils/asyncHandler.js')
const helpers=require( "../../utils/helpers.js");
const SocketIo=require('../../socket/index.js')
...
const deleteCascadeChatMessages = async (chatId) => {
  ...
  attachments.forEach((attachment) => {
    ...
    helpers.removeLocalFile(attachment.localPath);
  });
  ...
  const users = await User.aggregate([
    {
      $match: {
        _id: {
          $ne: req.user.userId, // avoid logged in user
        },
      },
    },
    {
...
const createOrGetAOneOnOneChat = asyncHandler(async (req, res) => {
  ...
  const newChatInstance = await Chat.create({
    ...
    participants: [req.user.userId, new mongoose.Types.ObjectId(receiverId)], // add receiver and logged in user as participants
  ...
  payload?.participants?.forEach((participant) => {
    if (participant._id.toString() === req.user.userId.toString()) return; // don't emit the event for the logged in use as he is the one who is initiating the chat

    // emit event to other participants with new chat as a payload
    SocketIo.emitSocketEvent(
      req,
...
const createAGroupChat = asyncHandler(async (req, res) => {
  ...
  if (participants.includes(req.user.userId.toString())) {
    throw new ApiError(
      400,
  ...
  const members = [...new Set([...participants, req.user.userId.toString()])]; // check for duplicates
...
const renameGroupChat = asyncHandler(async (req, res) => {
  ...
  payload?.participants?.forEach((participant) => {
    ...
    emitSocketEvent(
      req,
      participant._id?.toString(),
      constants.ChatEventEnum.UPDATE_GROUP_NAME_EVENT,
      payload
    );
  });
...
const deleteGroupChat = asyncHandler(async (req, res) => {
  ...
  chat?.participants?.forEach((participant) => {
    if (participant._id.toString() === req.user.userId.toString()) return; // don't emit the event for the logged in use as he is the one who is deleting
    // emit event to other participants with left chat as a payload
    emitSocketEvent(
      req,
      participant._id?.toString(),
      constants.ChatEventEnum.LEAVE_CHAT_EVENT,
      chat
    );
  });
...
const deleteOneOnOneChat = asyncHandler(async (req, res) => {
  ...
  const otherParticipant = payload?.participants?.find(
    (participant) => participant?._id.toString() !== req.user.userId.toString() // get the other participant in chat for socket
  );
...
const leaveGroupChat = asyncHandler(async (req, res) => {
  ...
  if (!existingParticipants?.includes(req.user?.userId)) {
  ...
  const updatedChat = await Chat.findByIdAndUpdate(
    ...
    {
      $pull: {
        participants: req.user?.userId, // leave the group
      },
    },
    { new: true }
  );
...
const addNewParticipantInGroupChat = asyncHandler(async (req, res) => {
  ...
  if (groupChat.admin?.toString() !== req.user.userId?.toString()) {
    throw new ApiError(404, "You are not an admin");
  }
...
const removeParticipantFromGroupChat = asyncHandler(async (req, res) => {
  ...
  SocketIo.emitSocketEvent(req, participantId, ChatEventEnum.LEAVE_CHAT_EVENT, payload);
...
const getAllChats = asyncHandler(async (req, res) => {
  const chats = await Chat.aggregate([
    {
      $match: {
        participants: { $elemMatch: { $eq: req.user.userId } }, // get all chats that have logged in user as a participant
      },
    },
    {
      $sort: {
