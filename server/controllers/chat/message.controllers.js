...
const sendMessage = asyncHandler(async (req, res) => {
  ...
  if (req.files && req.files.attachments?.length > 0) {
    req.files.attachments?.map((attachment) => {
      messageFiles.push({
        url: helpers.getStaticFilePath(req, attachment.filename),
        localPath: helpers.getLocalPath(attachment.filename),
      });
    });
  }
  ...
  chat.participants.forEach((participantObjectId) => {
    ...
    if (participantObjectId.toString() === req.user.userId.toString()) return;

    // emit the receive message event to the other participants with received message as the payload
    SocketIo.emitSocketEvent(
      req,
      participantObjectId.toString(),
      constants.ChatEventEnum.MESSAGE_RECEIVED_EVENT,
      receivedMessage
    );
  });
...
const deleteMessage = asyncHandler(async (req, res) => {
  ...
  if (message.attachments.length > 0) {
    ...
    message.attachments.map((asset) => {
      helpers.removeLocalFile(asset.localPath);
    });
  }
  ...
  chat.participants.forEach((participantObjectId) => {
    ...
    emitSocketEvent(
      req,
      participantObjectId.toString(),
      constants.ChatEventEnum.MESSAGE_DELETED_EVENT,
      message._id
    );
  });
